...
class RosBridgeClient:
    ...
    def _route_message(self, msg: dict):
        ...
        # Topic format: /robot/<base32>/response o /robot/<base32>/status
        parts = topic.split("/")
    ...
    async def publish_command(self, robot_id: UUID, command_payload: dict):
        """Publicar comando al topic /robot/<base32>/command."""
        b32 = uuid_to_crockford_base32(robot_id)
        msg = {
            "op": "publish",
            "topic": f"/robot/{b32}/command",
            "msg": {"data": json.dumps(command_payload)}
        }
        await self._ws.send(json.dumps(msg))

    async def subscribe_robot(self, robot_id: UUID, queue: asyncio.Queue):
    ...
    async def unsubscribe_robot(self, robot_id: UUID, queue: asyncio.Queue):
    ...
    async def _resubscribe_all(self):
        """Re-suscribirse a todos los robots tras una reconexión."""
        robots = list(self._subscribed_robots)
