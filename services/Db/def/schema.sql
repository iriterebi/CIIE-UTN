...
CREATE TABLE public.robots (
    ...
    status character varying(255) NOT NULL,
    user_id integer,
    CONSTRAINT robots_status_check CHECK (((status)::text = ANY ((ARRAY['pending_approval'::character varying, 'approved'::character varying, 'rejected'::character varying, 'disabled'::character varying])::text[])))
);
